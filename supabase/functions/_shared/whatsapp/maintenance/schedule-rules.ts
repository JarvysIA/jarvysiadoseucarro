// PORTA de src/lib/maintenance-jarvys-schedule-rules.ts — cópia deliberada, não reimportar
...
//
// Diferença deliberada em relação ao original: a seção "Detecção de perfil
// a partir do plano IA validado" (normalize/detectFuelKind/
// detectTransmissionKind/detectSteeringKind/inferJarvysProfileFromPlan) foi
// cortada nesta porta — nenhuma delas é usada pelo renderer de marco (que
// recebe o perfil já pronto, em vez de inferi-lo de um MaintenancePlanJson
// completo), e mantê-las exigiria portar toda a árvore de tipos de
// MaintenancePlanJson. O resto do corpo é cópia literal.
