// PORTA de src/lib/parse-receipt.functions.ts — cópia deliberada do
// SYSTEM_PROMPT e da lógica de chamada/parsing da IA de visão. Removida
// a camada de createServerFn/auth/paywall do TanStack (não existe no
// Deno) — o gate comercial (canVehiclePerformFullAction) e a
// integração com WhatsApp ficam para um build futuro (OCR-2). Se o
// prompt ou o modelo mudarem no app, este arquivo precisa ser
// atualizado manualmente em par.

export type ReceiptCategory = "oleo" | "filtros" | "pneus" | "freios" | "bateria" | "outro";
...
// Leitura de Deno.env via globalThis (mesmo padrão de actions/deps.ts e
// actions/expense-deps.ts) — evita ReferenceError ao importar este módulo
// fora do Deno (ex: bun test), já que `Deno` não existe como global no bun.
...
export async function parseReceiptImage(input: ParseReceiptInput): Promise<ParseReceiptResult> {
  if (!input?.imageBase64 || typeof input.imageBase64 !== "string") {
    return { ok: false, error: "imageBase64 é obrigatório" };
  }
  if (input.imageBase64.length > 12_000_000) {
    return { ok: false, error: "Imagem muito grande (máx ~9MB)." };
  }
  ...
  try {
    ...
    const itens = Array.isArray(parsed.itens_identificados)
      ? parsed.itens_identificados.map((it) => ({
          ...
          categoria: (["oleo", "filtros", "pneus", "freios", "bateria", "outro"].includes(
            String(it?.categoria),
          )
            ? it.categoria
            : "outro") as ReceiptCategory,
    ...
    return {
      ...
      receipt: {
        ...
        km_registrada: parsed.km_registrada == null ? null : Number(parsed.km_registrada) || null,
