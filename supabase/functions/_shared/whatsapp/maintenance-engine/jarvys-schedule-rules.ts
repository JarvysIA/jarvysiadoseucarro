...
// src/lib/maintenance-jarvys-schedule-rules.ts (2026-09-02), adaptada pro
// runtime Deno de supabase/functions/_shared/. Mesmo padrão de duplicação
// de arquivo puro já estabelecido no repo (ex: receipt-ocr/parse-receipt.ts
// entre src/lib e supabase/functions/_shared). Sem lógica alterada — cópia
// fiel de tudo que buildJarvysMilestone chama internamente.
//
// Omitido deliberadamente (não usado neste build, perfil já vem pronto do
// banco em veiculos.jarvys_technical_profile): inferJarvysProfileFromPlan,
// detectFuelKind, detectTransmissionKind, detectSteeringKind, normalize,
// toMaintenancePlanItem, buildJarvysScheduleRange,
// isDeterministicRevisionItem/DETERMINISTIC_REVISION_ITEM_KEYS. Esses
// dependiam de MaintenancePlanJson/MaintenancePlanItem (schema Zod estrito
// de src/lib/maintenance-plan-schema.ts, que não existe neste runtime).
//
// Os 6 tipos que o `JarvysItem` original importava de
// maintenance-plan-schema.ts (MaintenanceAction, MaintenanceCategory,
// MaintenanceRecommendationType, ShoppingClassification, TimingSystem,
// TransmissionType) são reproduzidos aqui como literais inline idênticos
// (conferidos linha a linha contra o arquivo fonte) — são só union types,
// zero valor de runtime, então inliná-los preserva a forma exata dos dados
// sem precisar duplicar o schema Zod inteiro.
//
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Tipos inline (equivalentes aos de maintenance-plan-schema.ts)
// ─────────────────────────────────────────────────────────────
