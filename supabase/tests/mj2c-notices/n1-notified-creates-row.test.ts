/**
 * MJ2C-V N1 — 1ª chamada com action='notified' cria linha nova, status
 * 'notified', notified_at preenchido.
 */
...
describeIfDb("MJ2C-V N1 — notified cria linha nova", () => {
  let setup: Session;
  ...
  test("1ª chamada com action='notified' → recorded, status notified, notified_at preenchido", async () => {
    const r = await setup.query<{ record_whatsapp_milestone_notice: Result }>(
      ...
      [SYNTH_USER_ID, SYNTH_VEHICLE_ID, 60000, "notified"],
    );
    ...
    expect(res?.snoozedUntil ?? null).toBeNull();
    expect(res?.dismissedAt ?? null).toBeNull();
...
if (!HAS_DB) {
  console.log("[MJ2C-V N1] skipped: TEST_DATABASE_URL ausente (esperado fora do CI)");
}
