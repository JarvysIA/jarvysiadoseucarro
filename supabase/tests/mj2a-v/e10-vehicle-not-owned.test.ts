/**
 * MJ2A-V E10 — vehicle_not_owned: draft aponta pra veículo do OUTRO
 * usuário, o contato pertence a SYNTH_USER_ID → rejected/vehicle_not_owned.
 */
...
const ORCH = "mj2av-e10-v1";

type Result = { kind: string; reason?: string };

describeIfDb("MJ2A-V E10 — vehicle_not_owned", () => {
  let setup: Session;
  ...
  test("veículo pertence ao OTHER_USER → rejected", async () => {
    ...
    const msgId = await seedConfirmationMessage(setup, "E10");
    ...
    const desp = await setup.query<{ n: string }>(
      `select count(*)::text as n from public.despesas
        where vehicle_id = $1`, [SYNTH_OTHER_VEHICLE_ID],
    );
...
if (!HAS_DB) {
  ...
  console.log("[MJ2A-V E10] skipped: TEST_DATABASE_URL ausente");
}
