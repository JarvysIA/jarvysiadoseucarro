/**
 * MJ2A-V E5 — Estado inválido (fora de awaiting_expense_*): 'idle' com
 * draft_type='expense' → rejected/invariant_violation.
 */
...
const ORCH = "mj2av-e5-v1";

type Result = { kind: string; reason?: string };

describeIfDb("MJ2A-V E5 — invalid state", () => {
  let setup: Session;
  ...
  test("state='idle' → rejected/invariant_violation", async () => {
    ...
    const msgId = await seedConfirmationMessage(setup, "E5 confirm");
...
if (!HAS_DB) {
  ...
  console.log("[MJ2A-V E5] skipped: TEST_DATABASE_URL ausente");
}
