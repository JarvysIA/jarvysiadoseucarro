/**
 * MJ2B-V — Cenário F2: rejected + retry_needed. Mirror do D2 do MJ1D-V,
 * adaptado (sem km_conflict — despesa usa vehicle_archived como motivo de
 * rejeição facilmente provocável). Prova, além do fluxo do apply(), que a
 * RPC execute_whatsapp_expense_create NÃO grava linha em
 * whatsapp_action_executions quando rejeita — fechando explicitamente uma
 * lacuna que o D2 do KM deixou em aberto.
 */
...
function buildCtxMock(
  ...
  return {
    ...
    activeVehicleIssue: "archived",
  };
}

describeIfDb("MJ2B-V F2 — rejected (vehicle_archived) + retry_needed", () => {
  let session: Session;
  ...
  test("veículo arquivado por fora -> RPC rejected, sem action_execution, apply retry_needed", async () => {
    ...
    expect(execResult?.kind).toBe("rejected");
    expect(execResult?.reason).toBe("vehicle_archived");
    ...
    expect(decision.responseKey).toBe("expense_create_retry_needed");
    ...
    // despesas NÃO ganhou linha — RPC rejeitou antes de qualquer INSERT.
    const afterCount = await fetchDespesaCount(session, SYNTH_VEHICLE_ID);
    expect(afterCount).toBe(0);

    // whatsapp_action_executions NÃO tem linha para este draft — a RPC de
    // despesa rejeita ANTES de gravar action_executions (diferente do KM,
    // aqui provamos explicitamente).
    ...
    expect(Number(execRows.rows[0]?.n)).toBe(0);

    const st = await session.query<{ state: string; draft_id: string | null }>(
...
if (!HAS_DB) {
  ...
  console.log("[MJ2B-V F2] skipped: TEST_DATABASE_URL ausente (esperado fora do CI)");
}
