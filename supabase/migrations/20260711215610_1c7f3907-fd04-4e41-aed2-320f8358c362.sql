-- Build 5.7F2B2C — Recriação da RPC atômica do orquestrador WhatsApp
-- Congelamento final conforme R1/R2/R3/R4.

DROP FUNCTION IF EXISTS public.apply_whatsapp_orchestrator_transition(
  uuid, uuid, bigint, jsonb, text, jsonb, jsonb
);

CREATE FUNCTION public.apply_whatsapp_orchestrator_transition(
...
DECLARE
  ...
  c_event_kinds      constant text[] := ARRAY['greeting','help','confirm','deny','cancel_task','reset_conversation','vehicle_reply','unknown','expired_state'];
...
BEGIN
  -- ============================================================
  -- v_call_now (A): pré-lock, validação estática de p_response
  -- ============================================================
  v_call_now := clock_timestamp();

  -- ============================================================
  -- Validação pré-lock: parâmetros básicos
  -- ============================================================
  ...
  -- ============================================================
  -- Validação pré-lock: p_result_summary (camelCase estrito)
  -- ============================================================
  ...
  -- Validate types of patch fields (casts guarded)
  BEGIN
  ...
  -- ============================================================
  -- Validação pré-lock: p_response (opcional)
  -- ============================================================
  ...
  SELECT id, contact_id, provider, instance_id, direction, message_type
    INTO v_msg_pre
  ...
  -- LOCK 1: contato
  -- ============================================================
  SELECT id, user_id, phone_e164, verified_at, unlinked_at
    INTO v_contact
  ...
  -- ============================================================
  -- Precedência: (2) Invariantes das marcas orchestrator_*
  -- ============================================================
  -- CHECK wpq_orchestrator_fields_consistent garante all-null OR all-not-null.
  -- Se qualquer marca preenchida sem estar em status='done'+replay → invariant.
  ...
  -- ============================================================
  -- Precedência: (3) Terminal sem replay
  -- ============================================================
  ...
  -- ============================================================
  -- Validação da mensagem (após replay/invariantes/terminal)
  -- ============================================================
  ...
  -- ============================================================
  -- Instância (resolver via message.provider + message.instance_id)
  -- ============================================================
  ...
  -- ============================================================
  -- v_lease_checked_at (B): validação real do lease pós-locks
  -- ============================================================
  ...
  ELSE
    ...
    v_snap_fallback    := 0;
    v_snap_ver         := 0;
    v_state_created_at := clock_timestamp(); -- (C)
  END IF;
  ...
  -- ============================================================
  -- Aplicar patch em memória: chave ausente preserva; null limpa
  -- ============================================================
  ...
  -- expires_at validation (usar v_state_created_at correto conforme real/virtual)
  ...
  -- ============================================================
  -- active_vehicle_id (ownership + not archived; sem gate de plano)
  -- ============================================================
  ...
  -- ============================================================
  -- Outbound idempotente (resolve existente)
  -- ============================================================
  v_outbound_id := NULL;
  IF p_response IS NOT NULL THEN
  ...
  -- ============================================================
  -- v_finished_at (D): antes da primeira escrita
  -- ============================================================
  v_finished_at := clock_timestamp();

  -- ============================================================
  -- ESCRITA 1: outbound (se necessário)
  -- ============================================================
  ...
  -- ============================================================
  -- ESCRITA 2: state (INSERT ou UPDATE com CAS)
  -- ============================================================
  ...
  -- ============================================================
  -- ESCRITA 3: orchestrator_result sanitizado
  -- ============================================================
  ...
  -- ============================================================
  -- ESCRITA 4: finalizar processing queue
  -- ============================================================
...
-- Grants
REVOKE ALL ON FUNCTION public.apply_whatsapp_orchestrator_transition(
  uuid, uuid, bigint, jsonb, text, jsonb, jsonb
) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.apply_whatsapp_orchestrator_transition(
  uuid, uuid, bigint, jsonb, text, jsonb, jsonb
...
GRANT EXECUTE ON FUNCTION public.apply_whatsapp_orchestrator_transition(
  uuid, uuid, bigint, jsonb, text, jsonb, jsonb
) TO service_role;