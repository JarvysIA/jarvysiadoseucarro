-- Build 5.7F2E1B-fix2 — corrige a ordem de validação em
-- public.execute_whatsapp_km_update. Assinatura idêntica (15 parâmetros),
-- SECURITY DEFINER, search_path=public. CREATE OR REPLACE preserva grants.
-- Mudanças:
--   1) Idempotência (whatsapp_action_executions) movida para logo APÓS a
--      validação do veículo, ANTES de qualquer recálculo de correção.
--   2) Nova checagem km_conflict: se km_atual real IS DISTINCT FROM
--      p_expected_previous_km, retorna imediatamente sem tocar em
--      whatsapp_action_executions.
--   3) Só após bater expected_previous_km é que segue para recálculo de
--      isCorrection real e correction_confirmed.
...
BEGIN
  ...
  -- ==========================================================
  -- (6) Idempotência via whatsapp_action_executions.
  -- Movida para ANTES da checagem de km_conflict e do recálculo de
  -- correção: se já houve execução prévia para esse draft, replay puro.
  -- ==========================================================
  ...
  -- ==========================================================
  -- (7) NOVO — km_conflict antes de qualquer avaliação de correção.
  -- Se o km real do veículo divergir do expected_previous_km do draft,
  -- retorna imediatamente sem gravar execução.
  -- ==========================================================
  IF v_current_km IS DISTINCT FROM p_expected_previous_km THEN
    RETURN jsonb_build_object(
      'kind','conflicted',
      'reason','km_conflict',
  ...
  -- ==========================================================
  -- (8) Revalida isCorrection contra o km real do veículo.
  -- ==========================================================
  ...
  -- ==========================================================
  -- (9) No-op: p_new_km igual ao km atual real.
  -- ==========================================================
  ...
  -- ==========================================================
  -- (10) UPDATE atômico condicionado ao km esperado.
  -- ==========================================================
...
) IS 'Build 5.7F2E1B-fix2 — RPC atômica/idempotente de aplicação confirmada de KM. Ordem: contato -> estado -> veículo -> idempotência -> km_conflict -> correção -> no_op -> UPDATE. Sem caller runtime.';
