...
// Substitui a camada 6.42C: agora as milestones do plano são
// SOBRESCRITAS pela matriz oficial Jarvys (10k–200k). A IA segue
// autora de vehicle_summary, system_profile, fixed_intervals,
// severe_use_rules, age_based_alerts, not_applicable_items,
// purchase_bundles, general_notes, safety_disclaimer e metadata.
// O conteúdo de milestones.items emitido pela IA é descartado.
//
// Schema estrito respeitado: itens internos do helper (group_key,
// requires_confirmation, revisionKmReal, revisionKmBase, cycleIndex,
// isHighMileage) NUNCA são inseridos em MaintenancePlanJson. A
// projeção é feita por `toMaintenancePlanItem`; auditoria interna
// vai para `notes` (campo aceito pelo Zod).
//
// Ciclo infinito acima de 200.000 km: o helper expõe `mapRealKmToBaseKm`
// e `buildJarvysMilestone(realKm, profile)`. O dry-run continua emitindo
// 10k–200k por compatibilidade com `validateMilestoneSchedule`; consumo
// >200k é responsabilidade de futuros consumidores (card de próxima
// revisão, payload do dashboard).
// ─────────────────────────────────────────────────────────────

import {
  JARVYS_BASE_CYCLE_MAX_KM,
  JARVYS_BASE_CYCLE_MIN_KM,
  buildJarvysScheduleRange,
  inferJarvysProfileFromPlan,
...
function applyJarvysDeterministicMaintenanceRules(
  ...
  warnings.push(
    `jarvys_profile:fuel=${profile.fuelKind};timing=${profile.timingSystem};transmission=${profile.transmissionKind};steering=${profile.steeringKind}`,
  );

  // Preserva notes e purchase_bundle_keys da IA por km.
  const aiByKm = new Map<
    number,
    { notes?: string[]; purchase_bundle_keys?: string[] }
  >();
  for (const m of plan.milestones) {
    aiByKm.set(m.km, {
      notes: m.notes,
      purchase_bundle_keys: m.purchase_bundle_keys,
    });
  }

  const jarvysMilestones = buildJarvysScheduleRange({
    fromKm: JARVYS_BASE_CYCLE_MIN_KM,
    toKm: JARVYS_BASE_CYCLE_MAX_KM,
    profile,
  });

  const nextMilestones = jarvysMilestones.map((jm) => {
    const aiExtras = aiByKm.get(jm.revisionKmReal);
    const items = jm.items.map(toMaintenancePlanItem);
    for (const it of items) {
      warnings.push(`jarvys_rule_applied:${jm.revisionKmReal}:${it.item_key}`);
    }
    const aiNotes = aiExtras?.notes ?? [];
    const mergedNotes = [...jm.notes, ...aiNotes];
    return {
      km: jm.revisionKmReal,
      label: jm.label,
      revision_number: jm.revisionNumber,
      items,
      ...(aiExtras?.purchase_bundle_keys
        ? { purchase_bundle_keys: aiExtras.purchase_bundle_keys }
        : {}),
